...
| DeepSeek Harness (`dsh-tui`) | Tracked submitted input | Visible-screen indicators only, never response text. The status bar's hint reads `esc to interrupt` (`esc 中断` in its Chinese UI). The working row above the composer is a breathing dot, one of dsh-TUI's 18 fixed spinner verbs and `…`, then nothing or the bracketed timer, token count or `thinking`, for example `● Thinking… (3s · ↓ 12 tokens)`. Both disappear when the turn ends, so status returns to IDLE | Derived from the dsh-TUI 0.11.0 source (`WorkingSpinner`, `SpinnerAnimationRow`, `StatusLine`, `spinnerVerbs`) and covered by inline unit cases. It has not been checked against a live session, because DeepSeek Harness is not installed on the development machine. dsh ships hook packages (`dsh-hooks-claude-code`, `dsh-hooks-codex`), but they are enabled only through the user's dsh profile config, which the launcher does not rewrite, so there is no scoped adapter. Restored cards start a new conversation (see `tmux::resolve_resume_command_with_session`) |
