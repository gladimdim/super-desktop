...
| Antigravity (`agy`) | `altScreenMode: "never"` in `~/.gemini/antigravity-cli/settings.json` (user setting; agy has no per-launch flag or environment variable for it) | 1.2.11: the default rendered its sign-in screen in the alternate screen; with `never` it stayed on the main screen. Signed-in replies not yet exercised. |
| Gemini (retired upstream), Cursor | Existing defaults | Startup/main screen only; login/onboarding prevents claiming full-response verification. |
...
| DeepSeek Harness (`dsh-tui`) | No flag (alternate screen by default) + remote paging | Checked against the 0.11.0 source on 2026-09-25; not installed here, so no live run. By default this Ink app draws in the alternate screen, so the phone gets only its current viewport and no scrollback, and PgUp/PgDn page the transcript. Turning off its **Fullscreen mode** setting (in `/settings`, or `fullscreen: false` in the profile's `cordis.yml`) switches it to inline rendering with native scrollback, which the phone can read. It takes effect after a restart. There is no dedicated CLI flag or `DSH_TUI_*` variable for it, so no launch flag is added. |
