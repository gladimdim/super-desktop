...
- **🔌 Phone bridge (optional)** — encrypted HTTPS/WSS over LAN or Tailscale (port 8759, mDNS `_omarchy-harness._tcp`). Desktop QR verification, explicit approval, and per-phone revocation live in the ⚙ gear → *Launcher connection*. Tailscale is optional. Protocol v3 requires re-pairing older phones. See [Security](SECURITY.md).
